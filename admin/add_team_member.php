...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($name) || empty($role) || !isset($_FILES['image']) || $_FILES['image']['error'] !== UPLOAD_ERR_OK) {
        ...
        $image_db_path = './uploads/team/' . $image_name; // Path to store in DB
        ...
        if (!in_array($image['type'], $allowed_types)) {
            ...
            if (empty($error)) {
                if (move_uploaded_file($image['tmp_name'], $target_path)) {
                    ...
                    try {
                        $stmt = $conn->prepare("INSERT INTO team_members (name, role, image, status) VALUES (?, ?, ?, ?)");
                        $stmt->bind_param("ssss", $name, $role, $image_db_path, $status);
                        if ($stmt->execute()) {
                            $_SESSION['success_message'] = 'Team member added successfully!';
                            header("Location: our_team.php");
                            exit();
                        } else {
                            $error = 'Failed to add team member: ' . $stmt->error;
